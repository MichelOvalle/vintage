...
def get_vintage_matrix(pref_num, pref_den, uen, filtros):
    ...
    df = duckdb.query(f"SELECT {cols} FROM '{FILE_PATH}' {where} GROUP BY 1 ORDER BY 1").df()
    if not df.empty:
        df = df.set_index('Mes originacion')
...
st.caption("Dashboard Vintage Pro v43.0 (Restaurada) | Michel Ovalle | Engine: DuckDB")