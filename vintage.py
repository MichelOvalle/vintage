...
# Normalizamos la fecha agregando el día -01 para que DuckDB la trate como DATE
...
def build_in_clause(filter_list):
    """Construye una clausula IN segura para SQL evitando errores de NoneType"""
...
def get_vintage_matrix(pref_num, pref_den, uen, filters):
    ...
    # Construcción de columnas: Capital Inicial + 24 Meses
...
try:
    if os.path.exists(FILE_PATH):
        ...
        with tab2:
            ...
            # 1. Curvas de Maduración (Últimas 8 cosechas)
            if not m_pr.empty:
            ...
            st.divider()
            # 2. Tendencias Globales (Solo últimos 24 meses)
            st.subheader("Evolución Global del Riesgo (Ventana 24 meses)")
            ...
            with col_g1:
                ...
                st.plotly_chart(px.line(duckdb.query(q_ev_pr).df(), x='Cosecha', y='Ratio', title="Ratio C2 Global - PR", markers=True).update_layout(yaxis_tickformat='.1%', plot_bgcolor='white'), use_container_width=True)
            ...
            with col_g2:
                ...
                st.plotly_chart(px.line(duckdb.query(q_ev_sol).df(), x='Cosecha', y='Ratio', title="Ratio C1 Global - SOLIDAR", markers=True, color_discrete_sequence=['red']).update_layout(yaxis_tickformat='.1%', plot_bgcolor='white'), use_container_width=True)

        with tab3:
            st.title("📍 Detalle de Desempeño")
            st.info("💡 Esta sección utiliza COALESCE para evitar errores de visualización por datos nulos.")
            
            # Matriz Sucursal vs Producto (Blindada contra NaNs)
            st.subheader("🔲 Matriz Sucursal vs Producto (Ratio C2 - PR)")
            q_pivot = f"""
                SELECT 
                    COALESCE(nombre_sucursal, 'N/A') as Sucursal, 
                    COALESCE(producto_agrupado, 'N/A') as Producto, 
                    sum(saldo_capital_total_c2)/NULLIF(sum(capital_c2), 0) as Ratio 
                FROM '{FILE_PATH}' WHERE uen='PR' GROUP BY 1, 2
            """
            df_pivot_raw = duckdb.query(q_pivot).df()
            if not df_pivot_raw.empty:
                df_p = df_pivot_raw.pivot(index='Sucursal', columns='Producto', values='Ratio').fillna(0)
        ...
        st.error(f"Error: No se encontró el archivo '{FILE_PATH}' en el repositorio.")

except Exception as e:
    st.error(f"Error técnico detectado: {e}")

st.caption("Dashboard Analítico de Crédito | Michel Ovalle | Engine: DuckDB")