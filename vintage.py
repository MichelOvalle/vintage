...
try:
    if os.path.exists(FILE_PATH):
        ...
        with tab1:
            ...
            for uen, p_num, p_den, tit in [('PR', 'saldo_capital_total_c', 'capital_c', 'Ratio 30-150'), 
                ...
                if not m_v.empty:
                    ...
                    # Aplicamos el estilo restaurado: semáforo con texto negro forzado
                    st.dataframe(
        ...
        with tab2:
            ...
            # 1. Curvas de Maduración (18 meses con leyenda inferior)
            ...
            # Evolución Global Vertical (PR y SOLIDAR)
            for uen, col_r, col_c, tit_u, line_c in [('PR', 'saldo_capital_total_c2', 'capital_c2', 'PR', 'blue'), 
                ...
                # Top Productos Críticos Reales (Filtro v40)
                qn = f"""
                    SELECT producto_agrupado as P FROM '{FILE_PATH}' {t_f} AND uen='{uen}' 
                    AND producto_agrupado IS NOT NULL AND producto_agrupado != '' AND UPPER(producto_agrupado) != 'SIN NOMBRE'
                    GROUP BY 1 ORDER BY sum({col_r})/NULLIF(sum({col_c}), 0) DESC LIMIT 4
                """
        ...
        with tab3:
            ...
            st.divider()
            # 2. Matrices Cruzadas (Sucursal vs Producto)
